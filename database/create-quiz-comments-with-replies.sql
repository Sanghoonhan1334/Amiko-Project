-- 댓글과 답글을 위한 완전한 SQL 테이블 시스템
-- Complete SQL table system for comments and replies

-- 1. 퀴즈 댓글 테이블 (메인 댓글과 답글 모두 저장)
CREATE TABLE IF NOT EXISTS public.quiz_comments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  quiz_id UUID NOT NULL REFERENCES public.quizzes(id) ON DELETE CASCADE,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  content TEXT NOT NULL,
  parent_id UUID REFERENCES public.quiz_comments(id) ON DELETE CASCADE, -- 답글용 (NULL이면 메인 댓글)
...
SELECT 'Quiz comments and replies system created successfully!' as status;
