-- =====================================================
-- 포인트 시스템 마이그레이션 스크립트 (Supabase용)
-- Description: Supabase SQL Editor에서 직접 실행 가능
-- Date: 2024-12-19
-- =====================================================
...
-- 3. 사용자 포인트 테이블 (이미 존재할 수 있으므로 IF NOT EXISTS 사용)
...
DROP POLICY IF EXISTS "Users can view own daily limit" ON public.daily_points_limit;
DROP POLICY IF EXISTS "System can manage all points" ON public.user_points;
DROP POLICY IF EXISTS "System can manage all points history" ON public.points_history;
DROP POLICY IF EXISTS "System can manage all daily limits" ON public.daily_points_limit;
...
-- 11. 포인트 사용 함수 (available_points 차감)
...
-- 12. 포인트 히스토리 삽입 시 자동으로 사용자 포인트 업데이트
...
-- 13. 초기 데이터 삽입 (기존 사용자들을 위한 기본 포인트 설정)
INSERT INTO public.user_points (user_id, available_points, total_points)
SELECT id, 0, 0
FROM public.users
