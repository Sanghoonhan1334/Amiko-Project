-- quizzes 테이블과 관련된 모든 테이블을 TEXT로 변경 (v3)
-- 실제 테이블 구조에 맞춤
...
-- 3. quiz_results.quiz_id를 TEXT로 변경 (있다면)
DO $$
BEGIN
...
-- quiz_results 외래 키 (있다면)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'quiz_results') THEN
        EXECUTE 'ALTER TABLE quiz_results ADD CONSTRAINT quiz_results_quiz_id_fkey FOREIGN KEY (quiz_id) REFERENCES quizzes(id) ON DELETE CASCADE';
    END IF;
END $$;

-- 9. 한국어 레벨 테스트 추가 (실제 컬럼만 사용)
INSERT INTO quizzes (
  id, 
  title, 
  description,
