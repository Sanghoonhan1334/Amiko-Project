...
-- RLS (Row Level Security) 정책 (이미 있으면 무시)
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'galleries' AND policyname = 'Anyone can view galleries') THEN
    CREATE POLICY "Anyone can view galleries" ON galleries FOR SELECT USING (true);
  END IF;
END $$;

-- 갤러리 게시물 정책 (이미 있으면 무시)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'gallery_posts' AND policyname = 'Anyone can view gallery posts') THEN
    CREATE POLICY "Anyone can view gallery posts" ON gallery_posts FOR SELECT USING (is_deleted = false);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'gallery_posts' AND policyname = 'Users can create gallery posts') THEN
    CREATE POLICY "Users can create gallery posts" ON gallery_posts FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'gallery_posts' AND policyname = 'Users can update own gallery posts') THEN
    CREATE POLICY "Users can update own gallery posts" ON gallery_posts FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'gallery_posts' AND policyname = 'Users can delete own gallery posts') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'gallery_comments' AND policyname = 'Anyone can view gallery comments') THEN
    CREATE POLICY "Anyone can view gallery comments" ON gallery_comments FOR SELECT USING (is_deleted = false);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'gallery_comments' AND policyname = 'Users can create gallery comments') THEN
    CREATE POLICY "Users can create gallery comments" ON gallery_comments FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'gallery_comments' AND policyname = 'Users can update own gallery comments') THEN
    CREATE POLICY "Users can update own gallery comments" ON gallery_comments FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'gallery_comments' AND policyname = 'Users can delete own gallery comments') THEN
    CREATE POLICY "Users can delete own gallery comments" ON gallery_comments FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;

-- 갤러리 좋아요 정책 (이미 있으면 무시)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'gallery_likes' AND policyname = 'Anyone can view gallery likes') THEN
    CREATE POLICY "Anyone can view gallery likes" ON gallery_likes FOR SELECT USING (true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'gallery_likes' AND policyname = 'Users can create gallery likes') THEN
    CREATE POLICY "Users can create gallery likes" ON gallery_likes FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'gallery_likes' AND policyname = 'Users can delete own gallery likes') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_gallery_posts_updated_at') THEN
    CREATE TRIGGER update_gallery_posts_updated_at BEFORE UPDATE ON gallery_posts
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_gallery_comments_updated_at') THEN
    CREATE TRIGGER update_gallery_comments_updated_at BEFORE UPDATE ON gallery_comments
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
END $$;

-- 갤러리 게시물 수 자동 업데이트 함수 (이미 있으면 교체)
...
-- 갤러리 게시물 수 트리거 (이미 있으면 무시)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'gallery_post_count_trigger') THEN
...
-- 갤러리 댓글 수 트리거 (이미 있으면 무시)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'gallery_comment_count_trigger') THEN
...
-- 갤러리 좋아요 수 자동 업데이트 함수 (이미 있으면 교체)
...
-- 갤러리 좋아요 수 트리거 (이미 있으면 무시)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'gallery_like_count_trigger') THEN
