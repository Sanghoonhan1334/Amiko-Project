...
import {
  ...
  MessageCircle, Hand, ScreenShare, ScreenShareOff,
  Heart, ThumbsUp, PartyPopper, Smile,
  Send, Clock, Users, X, Maximize2, Minimize2,
  AlertTriangle
} from 'lucide-react'
...
export default function VideoRoom({
  channel, token, uid, appId, sessionId, title,
  durationMinutes = 30, onLeave
}: VideoRoomProps) {
  const { t } = useLanguage()
  const clientRef = useRef<IAgoraRTCClient | null>(null)
  const localVideoRef = useRef<HTMLDivElement>(null)
  const screenTrackRef = useRef<ILocalVideoTrack | null>(null)
  const supabaseChannelRef = useRef<ReturnType<ReturnType<typeof createSupabaseBrowserClient>['channel']> | null>(null)
  ...
  useEffect(() => {
    ...
    realtimeChannel
      ...
      .on('broadcast', { event: 'reaction' }, ({ payload }) => {
        ...
        setFloatingReactions(prev => [...prev, { id: reactionId, emoji: payload.emoji }])
        setTimeout(() => {
        ...
        <div className={`grid ${gridCols} gap-2 h-full`}>
          ...
          <div className="relative rounded-lg overflow-hidden bg-gray-800">
            ...
            {!isVideoOn && (
              <div className="absolute inset-0 flex items-center justify-center bg-gray-800">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center">
                  <span className="text-2xl text-white font-bold">{uid.toString().charAt(0)}</span>
                </div>
              </div>
            )}
            <div className="absolute bottom-2 left-2 flex items-center gap-1">
              ...
              {!isAudioOn && (
                <Badge variant="secondary" className="bg-red-500/70 text-white text-[10px]">
      ...
      {showChat && (
        <div className="absolute right-0 top-12 bottom-20 w-80 bg-gray-800/95 backdrop-blur-sm border-l border-gray-700 flex flex-col z-10">
                ...
                {msg.type === 'system' ? (
                  ...
                  <div className={`rounded-lg px-2.5 py-1.5 ${
                    ...
                    <span className={`text-[10px] font-medium block ${
                      msg.senderUid === uid ? 'text-purple-300' : 'text-blue-400'
                    }`}>
                      {msg.senderUid === uid ? t('vcMarketplace.videoRoom.you') : msg.sender}
                    </span>
          ...
          <div className="p-2 border-t border-gray-700 flex gap-2">
            ...
            />
            <Button size="sm" onClick={handleSendChat} className="h-8 w-8 p-0 bg-purple-500 hover:bg-purple-600">
      ...
      <div className="flex items-center justify-center gap-3 px-4 py-3 bg-gray-800/80 backdrop-blur-sm">
        <button
          ...
          className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${
            isAudioOn ? 'bg-gray-600 hover:bg-gray-500 text-white' : 'bg-red-500 hover:bg-red-600 text-white'
          }`}
        >
        ...
        <button
          ...
          className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${
            isVideoOn ? 'bg-gray-600 hover:bg-gray-500 text-white' : 'bg-red-500 hover:bg-red-600 text-white'
          }`}
        >
        ...
        <button
          ...
          className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${
            isScreenSharing ? 'bg-green-500 hover:bg-green-600 text-white' : 'bg-gray-600 hover:bg-gray-500 text-white'
          }`}
        >
        ...
        <button
          ...
          className={`relative w-12 h-12 rounded-full flex items-center justify-center transition-colors ${
            showChat ? 'bg-purple-500 hover:bg-purple-600 text-white' : 'bg-gray-600 hover:bg-gray-500 text-white'
          }`}
        >
      ...
      {!user.videoTrack && (
        <div className="absolute inset-0 flex items-center justify-center bg-gray-800">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center">
            <span className="text-2xl text-white font-bold">{user.uid.toString().charAt(0)}</span>
          </div>
        </div>
      )}
