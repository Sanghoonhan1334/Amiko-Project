...
export default function CreateSessionModal({
  ...
  const handleSubmit = async () => {
    if (!title.trim()) {
      setError(t("vcMarketplace.createForm.errorTitle"));
      return;
    }
    if (!topic.trim()) {
      setError(t("vcMarketplace.createForm.errorTopic"));
      return;
    }
    if (!scheduledDate) {
      setError(t("vcMarketplace.createForm.errorDate"));
      return;
    }
    if (!scheduledTime) {
      setError(t("vcMarketplace.createForm.errorTime"));
      return;
    }

    const price = parseFloat(priceUsd);
    if (isNaN(price) || price < 0) {
      setError(t("vcMarketplace.createForm.errorPrice"));
      return;
    }

    const maxP = parseInt(maxParticipants);
    if (isNaN(maxP) || maxP < 2 || maxP > 20) {
      setError(t("vcMarketplace.createForm.errorParticipants"));
      return;
    }
            ...
            <div>
              ...
              {availableSlots.length > 0 ? (
                <Select value={scheduledTime} onValueChange={setScheduledTime}>
                  <SelectTrigger className="h-9 text-xs">
                    <SelectValue
                      placeholder={t("vcMarketplace.createForm.selectTime")}
                    />
                  </SelectTrigger>
                  <SelectContent>
