...
export default function PaymentsTab() {
  ...
  const groupedPurchases = paymentHistory?.reduce((acc, purchase) => {
      ...
      <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-3">
        ...
        <Card className="relative">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              ...
              <div className="min-h-[3rem] flex items-center">
                <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                  {selectedCoupon
                    ? couponOptions.find(opt => opt.id === selectedCoupon)?.description
              ...
              {selectedCoupon && (
                <div className="flex items-center justify-center">
                  <Badge variant="secondary" className="text-lg font-bold px-4 py-2">
                    ${couponOptions.find(opt => opt.id === selectedCoupon)?.amount}
                  </Badge>
                </div>
              )}
            </div>
            ...
            {selectedCoupon && (
              <PayPalPaymentButton
                amount={couponOptions.find(opt => opt.id === selectedCoupon)?.amount || 0}
                orderId={couponOptions.find(opt => opt.id === selectedCoupon)?.orderId || ''}
                orderName={couponOptions.find(opt => opt.id === selectedCoupon)?.orderName || ''}
                customerName={user?.user_metadata?.full_name || user?.email || ''}
                customerEmail={user?.email || ''}
                userId={user?.id || ''}
                productType={couponOptions.find(opt => opt.id === selectedCoupon)?.productType || ''}
                productData={couponOptions.find(opt => opt.id === selectedCoupon)?.productData || {}}
                className="w-full"
              />
            )}
          </CardContent>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              ...
              <div className="min-h-[3rem] flex items-center">
                <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                  {selectedVip
                    ? vipOptions.find(opt => opt.id === selectedVip)?.description
            ...
            {selectedVip && (
              <PayPalPaymentButton
                amount={vipOptions.find(opt => opt.id === selectedVip)?.amount || 0}
                orderId={vipOptions.find(opt => opt.id === selectedVip)?.orderId || ''}
                orderName={vipOptions.find(opt => opt.id === selectedVip)?.orderName || ''}
                customerName={user?.user_metadata?.full_name || user?.email || ''}
                customerEmail={user?.email || ''}
                userId={user?.id || ''}
                productType={vipOptions.find(opt => opt.id === selectedVip)?.productType || ''}
                productData={vipOptions.find(opt => opt.id === selectedVip)?.productData || {}}
                className="w-full"
              />
            )}
          </CardContent>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">
                              {purchase.product_data?.coupon_minutes ? `${purchase.product_data.coupon_minutes}분 쿠폰` : '쿠폰 구매'}
                            </p>
                        ...
                        <div className="text-right">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <Badge
                              ...
                              className={purchase.status === 'paid'
                          ...
                          <p className="text-xs text-gray-500">Order: {purchase.order_id}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    {groupedPurchases.vip_subscription.map((purchase) => (
                      <div key={purchase.id} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">
                              {purchase.product_data?.plan_type === 'yearly' ? 'VIP 연간 구독' :
                               purchase.product_data?.plan_type === 'lifetime' ? 'VIP 평생 구독' : 'VIP 월간 구독'}
                            </p>
                            <p className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-1">
                              <Calendar className="w-3 h-3" />
                              {new Date(purchase.created_at).toLocaleDateString()}
                        ...
                        <div className="text-right">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            >
                              {purchase.status === 'paid' ? t('payments.statusPaid') : t('payments.statusPending')}
                            </Badge>
                          </div>
                          <p className="text-xs text-gray-500">Order: {purchase.order_id}</p>
                        </div>
                    ...
                    {groupedPurchases.lecture.map((purchase) => (
                      <div key={purchase.id} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">
                              {purchase.product_data?.lecture_title || '강의 구매'}
                            </p>
                            <p className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-1">
                              <Calendar className="w-3 h-3" />
                              {new Date(purchase.created_at).toLocaleDateString()}
