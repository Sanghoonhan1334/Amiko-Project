...
import { Info, Upload, Share2, Play, Heart, MessageCircle, Eye } from 'lucide-react'
  ...
  const loadDanceData = async () => {
    ...
    try {
      ...
      if (playlistResponse.ok) {
        ...
        console.error('[DanceTab] 플레이리스트 조회 실패:', playlistResponse.status)
        setDancePlaylist(null)
      }
      
      // 승인된 댄스 비디오 가져오기 (최신 12개, 가이드 영상 포함)
      const videosResponse = await fetch('/api/dance/videos?limit=12&status=approved&includeGuide=true')
      if (videosResponse.ok) {
        ...
        if (videosData.success && videosData.videos) {
          ...
          console.warn('[DanceTab] 비디오 데이터가 없거나 success가 false:', videosData)
          setDanceVideos([])
        }
      } else {
        const errorData = await videosResponse.json().catch(() => ({}))
        console.error('[DanceTab] 비디오 조회 실패:', videosResponse.status, errorData)
        setDanceVideos([])
      }
    } catch (error) {
            ...
            return (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-1 sm:p-2 flex items-center gap-1.5 sm:gap-3 text-white text-[10px] sm:text-xs">
                  ...
                  >
                    <Heart className={`w-2.5 h-2.5 sm:w-3 sm:h-3 ${isLiked ? 'fill-red-500 text-red-500' : 'text-red-500'}`} />
