...
export default memo(function PointsCard() {
  const { t } = useLanguage();
  const { user } = useAuth();
  ...
  // VIP 남은 시간 계산 (memoized)
  const vipStatus = useMemo(() => {
    if (!vipExpiryDate) {
      return {
        text: t("storeTab.pointCard.vipInactive"),
    ...
    if (diffTime <= 0) {
      return {
        text: t("storeTab.pointCard.vipExpired"),
    ...
    if (diffDays === 1) {
      return {
        text: t("storeTab.pointCard.vipOneDayLeft"),
        color: "text-orange-500 dark:text-orange-400",
      };
    } else if (diffDays <= 7) {
      return {
        text: `${diffDays}${t("storeTab.pointCard.vipDaysLeft")}`,
        color: "text-orange-500 dark:text-orange-400",
      };
    } else {
      return {
    ...
    <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-4 mb-6">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="text-center p-2 sm:p-3 bg-white dark:bg-gray-700 rounded-lg border border-blue-200 dark:border-blue-600">
          {pointsLoading ? (
            <div className="text-lg font-bold text-blue-600 dark:text-blue-400 animate-pulse">
              ...
            </div>
          ) : (
          ...
          <div className="text-xs text-gray-600 dark:text-gray-300 mt-1">
            {t("storeTab.pointCard.availableAKO")}
          </div>
        </div>
        {/* 현재 포인트 카드 - 숨김 처리 */}
        ...
        <div className="text-center p-2 sm:p-3 bg-white dark:bg-gray-700 rounded-lg border border-orange-200 dark:border-orange-600 relative">
          ...
          {pointsLoading ? (
            ...
            <div
              className={`text-sm sm:text-lg font-bold ${vipStatus.color} mt-2 truncate`}
            >
              {vipStatus.text}
            </div>
          )}
          <div className="text-xs text-gray-600 dark:text-gray-300 mt-1">
            {t("storeTab.pointCard.vipStatus")}
          </div>
        </div>
      </div>
    </div>
