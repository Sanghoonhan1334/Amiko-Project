...
export default function BottomTabNavigation() {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      // Si la altura visible es significativamente menor que la ventana, el teclado está abierto
      ...
      <div className="grid grid-cols-5 py-1.5">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <Icon
                className={`w-5 h-5 mb-0.5 transition-transform duration-200 ${isActive ? "text-purple-600 dark:text-purple-400 scale-110" : "text-blue-600 dark:text-blue-400"}`}
              />
              <span
                className={`${t("language") === "es" ? "text-[9px]" : "text-[8px] xs:text-[9px] sm:text-[10px]"} font-bold whitespace-nowrap ${isActive ? "text-purple-600 dark:text-purple-400" : "text-blue-600 dark:text-blue-400"}`}
              >
                {tab.label}
              </span>
            </button>
