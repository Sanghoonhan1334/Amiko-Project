...
function getTwilioClient(): Twilio {
  if (!twilioClient) {
    ...
    if (!accountSid || !authToken) {
      throw new Error(
        "Twilio 계정 정보가 설정되지 않았습니다. TWILIO_ACCOUNT_SID와 TWILIO_AUTH_TOKEN을 환경변수에 설정해주세요.",
      );
    }
...
export async function sendTwilioSMS(
  ...
  try {
    ...
    try {
      ...
      if (
        accountInfo.isValid &&
        accountInfo.phoneNumbers &&
        accountInfo.phoneNumbers.length > 0
      ) {
        console.log(
          `[TWILIO_SMS] 계정에 등록된 번호 목록: ${accountInfo.phoneNumbers.join(", ")}`,
        );

        // 환경 변수에 설정된 번호가 계정에 등록되어 있는지 확인
        if (fromNumber && accountInfo.phoneNumbers.includes(fromNumber)) {
          console.log(
            `[TWILIO_SMS] ✅ 환경변수 번호(${fromNumber})가 계정에 등록되어 있음`,
          );
        } else {
          // 환경변수 번호가 계정에 없으면, 계정에 등록된 첫 번째 번호 사용
          fromNumber = accountInfo.phoneNumbers[0];
          console.log(
            `[TWILIO_SMS] ⚠️  환경변수 번호가 계정에 없어서 계정의 첫 번째 번호 사용: ${fromNumber}`,
          );
        }
      } else {
        // 계정에 등록된 번호가 없을 때
        if (fromNumber) {
          console.error(
            `[TWILIO_SMS] ❌ 계정에 등록된 번호가 없지만 환경변수 번호(${fromNumber})가 설정되어 있습니다.`,
          );
          console.error(
            `[TWILIO_SMS] 이 번호는 현재 계정에 등록되어 있지 않아 발송이 실패할 수 있습니다.`,
          );
          console.error(`[TWILIO_SMS] 해결 방법:`);
          console.error(
            `[TWILIO_SMS] 1. Twilio 콘솔에서 전화번호를 구매/등록하세요: https://console.twilio.com/`,
          );
          console.error(
            `[TWILIO_SMS] 2. 또는 .env.local의 TWILIO_PHONE_NUMBER를 계정에 등록된 번호로 변경하세요.`,
          );
        } else {
          console.error(
            `[TWILIO_SMS] ❌ 계정에 등록된 번호가 없고 환경변수 번호도 설정되지 않았습니다.`,
          );
          console.error(
            `[TWILIO_SMS] Twilio 콘솔에서 전화번호를 구매/등록하거나 .env.local에 TWILIO_PHONE_NUMBER를 설정하세요.`,
          );
        }
      }
    } catch (accountError) {
      console.warn(
        `[TWILIO_SMS] 계정 정보 조회 실패, 환경변수 번호 사용: ${fromNumber}`,
        accountError,
      );
    }
    ...
    if (
      ...
      if (!accountInfo.phoneNumbers.includes(fromNumber)) {
        console.error(
          `[TWILIO_SMS] ⚠️  경고: 환경변수 번호(${fromNumber})가 계정에 등록되어 있지 않습니다.`,
        );
        console.error(
          `[TWILIO_SMS] 등록된 번호: ${accountInfo.phoneNumbers.join(", ")}`,
        );
        console.error(
          `[TWILIO_SMS] 발송 시도는 하겠지만, Twilio API에서 에러가 발생할 수 있습니다.`,
        );
      }
    }
    ...
    if (to.startsWith("+56") && process.env.TWILIO_PHONE_NUMBER_CL) {
      ...
      try {
        ...
        if (
          accountInfo.phoneNumbers?.includes(process.env.TWILIO_PHONE_NUMBER_CL)
        ) {
          fromNumber = process.env.TWILIO_PHONE_NUMBER_CL;
          console.log(
            `[TWILIO_SMS] ✅ 칠레 → 칠레 로컬 번호 사용: ${fromNumber}`,
          );
        }
      } catch (e) {
        // 무시하고 기본 번호 사용
    ...
    // 멕시코 번호가 있고, 받는 사람이 멕시코면 멕시코 번호 사용
      ...
      try {
        const accountInfo = await verifyTwilioAccount();
        if (
          accountInfo.phoneNumbers?.includes(process.env.TWILIO_PHONE_NUMBER_MX)
        ) {
          fromNumber = process.env.TWILIO_PHONE_NUMBER_MX;
          console.log(
            `[TWILIO_SMS] ✅ 멕시코 → 멕시코 로컬 번호 사용: ${fromNumber}`,
          );
        }
      } catch (e) {
        // 무시하고 기본 번호 사용
    ...
    // 페루 번호가 있고, 받는 사람이 페루면 페루 번호 사용
      ...
      try {
        const accountInfo = await verifyTwilioAccount();
        if (
          accountInfo.phoneNumbers?.includes(process.env.TWILIO_PHONE_NUMBER_PE)
        ) {
          fromNumber = process.env.TWILIO_PHONE_NUMBER_PE;
          console.log(
            `[TWILIO_SMS] ✅ 페루 → 페루 로컬 번호 사용: ${fromNumber}`,
          );
        }
      } catch (e) {
        // 무시하고 기본 번호 사용
    ...
    if (!fromNumber) {
      console.error(
        `[TWILIO_SMS] ❌ Twilio 발신번호가 설정되지 않음 - 발송 불가`,
      );
      console.error(`[TWILIO_SMS] 수신번호: ${to}`);
      console.error(`[TWILIO_SMS] 환경변수 TWILIO_PHONE_NUMBER를 확인하세요.`);
      throw new Error(
        "Twilio 발신번호(TWILIO_PHONE_NUMBER)가 설정되지 않았습니다. .env.local 파일을 확인하세요.",
      );
    }
    ...
    if (error?.code === 21660) {
      console.error(
        "[TWILIO_SMS] ⚠️  에러 21660: 발신번호와 계정이 일치하지 않습니다.",
      );
      console.error("[TWILIO_SMS] 해결 방법:");
      console.error(
        "[TWILIO_SMS] 1. Twilio 콘솔(https://console.twilio.com/)에서 현재 계정의 전화번호를 확인하세요.",
      );
      console.error(
        "[TWILIO_SMS] 2. .env.local의 TWILIO_PHONE_NUMBER를 계정에 등록된 번호로 변경하세요.",
      );
      console.error(
        "[TWILIO_SMS] 3. 또는 Twilio 콘솔에서 전화번호를 구매/등록하세요.",
      );
    }
    ...
    console.error("[TWILIO_SMS] ========================================");
    // 실제 에러를 상위로 전달 (에러 원인을 숨기지 않음)
    const twilioError = new Error(
      `Twilio SMS 발송 실패: ${error?.message || "알 수 없는 오류"}${error?.code ? ` (코드: ${error.code})` : ""}${error?.moreInfo ? ` - ${error.moreInfo}` : ""}`,
    );
    (twilioError as any).twilioCode = error?.code;
    (twilioError as any).twilioStatus = error?.status;
    throw twilioError;
  }
}
  ...
  try {
    ...
    if (whatsappFromEnv) {
      ...
      if (whatsappFromEnv.startsWith("whatsapp:")) {
        ...
        console.log(
          `[TWILIO_WHATSAPP] ✅ 환경변수 번호 사용 (whatsapp: 접두사 포함): ${whatsappFrom}`,
        );
      } else {
        whatsappFrom = `whatsapp:${whatsappFromEnv}`;
        console.log(
          `[TWILIO_WHATSAPP] ✅ 환경변수 번호 사용 (whatsapp: 접두사 추가): ${whatsappFrom}`,
        );
      }

      // Sandbox 번호 사용 방지 (프로덕션 번호만 허용)
      if (
        whatsappFrom.includes("14155238886") ||
        whatsappFrom.includes("4155238886")
      ) {
        console.error(
          `[TWILIO_WHATSAPP] ❌ Sandbox 번호 사용 금지! 프로덕션 번호를 사용하세요.`,
        );
        console.error(`[TWILIO_WHATSAPP] 프로덕션 번호: whatsapp:+15557803562`);
        throw new Error(
          "Sandbox 번호는 사용할 수 없습니다. TWILIO_WHATSAPP_NUMBER를 프로덕션 번호(+15557803562)로 설정하세요.",
        );
      }
    } else {
      ...
      console.error(
        `[TWILIO_WHATSAPP] Vercel 환경 변수에 TWILIO_WHATSAPP_NUMBER를 설정하세요.`,
      );
      console.error(
        `[TWILIO_WHATSAPP] 프로덕션 번호: whatsapp:+15557803562 또는 +15557803562`,
      );
      throw new Error(
        "TWILIO_WHATSAPP_NUMBER 환경 변수가 설정되지 않았습니다. 프로덕션 번호(+15557803562)를 설정하세요.",
      );
    }
    ...
    console.log(
      `[TWILIO_WHATSAPP] 에러 메시지: ${result.errorMessage || "없음"}`,
    );
    console.log(
      `[TWILIO_WHATSAPP] 메시지 상세 확인: https://console.twilio.com/us1/monitor/logs/messages/${result.sid}`,
    );

    // 상태가 queued인 경우 경고
    if (result.status === "queued") {
      console.warn(
        `[TWILIO_WHATSAPP] ⚠️  메시지가 큐에 들어갔습니다. 실제 전송 여부는 Twilio 콘솔에서 확인하세요.`,
      );
      console.warn(
        `[TWILIO_WHATSAPP] ⚠️  Sandbox 모드인 경우 수신 번호가 Sandbox에 등록되어 있어야 합니다.`,
      );
      console.warn(
        `[TWILIO_WHATSAPP] ⚠️  Sandbox 등록: https://console.twilio.com/us1/develop/sms/sandbox`,
      );
    }
    ...
    if (result.errorCode) {
      ...
      if (result.errorCode === 63007) {
        console.error(
          `[TWILIO_WHATSAPP] ❌ 에러 63007: 수신 번호가 Sandbox에 등록되지 않았습니다.`,
        );
        console.error(`[TWILIO_WHATSAPP] 해결 방법:`);
        console.error(
          `[TWILIO_WHATSAPP] 1. Twilio 콘솔에서 Sandbox 설정 확인: https://console.twilio.com/us1/develop/sms/sandbox`,
        );
        console.error(
          `[TWILIO_WHATSAPP] 2. 수신 번호 ${whatsappTo.replace("whatsapp:", "")}를 Sandbox에 등록하세요.`,
        );
        console.error(
          `[TWILIO_WHATSAPP] 3. 또는 프로덕션 WhatsApp Business API로 전환하세요.`,
        );
      } else if (result.errorCode === 63015) {
        console.error(
          `[TWILIO_WHATSAPP] ❌ 에러 63015: Sandbox 채널은 Sandbox에 가입한 전화번호로만 메시지를 보낼 수 있습니다.`,
        );
        console.error(`[TWILIO_WHATSAPP] 해결 방법:`);
        console.error(`[TWILIO_WHATSAPP] 1. 수신 번호를 Sandbox에 등록:`);
        console.error(
          `[TWILIO_WHATSAPP]    - Twilio 콘솔 → Messaging → Try it out → Send a WhatsApp message`,
        );
        console.error(
          `[TWILIO_WHATSAPP]    - Sandbox에 표시된 코드를 수신 번호로 WhatsApp으로 보내기`,
        );
        console.error(
          `[TWILIO_WHATSAPP]    - 예: "join [코드]" 메시지를 +1 415 523 8886으로 보내기`,
        );
        console.error(
          `[TWILIO_WHATSAPP] 2. 또는 프로덕션 WhatsApp Business API로 전환 (권장):`,
        );
        console.error(
          `[TWILIO_WHATSAPP]    - Twilio 콘솔 → Messaging → Settings → WhatsApp Senders`,
        );
        console.error(
          `[TWILIO_WHATSAPP]    - WhatsApp Business API 승인 및 번호 등록`,
        );
        console.error(`[TWILIO_WHATSAPP]    - Content Template 생성 및 승인`);
      } else if (result.errorCode === 63016) {
        console.error(
          `[TWILIO_WHATSAPP] ❌ 에러 63016: 24시간 이내에 사용자가 메시지를 보내지 않았습니다.`,
        );
        console.error(`[TWILIO_WHATSAPP] 해결 방법:`);
        console.error(
          `[TWILIO_WHATSAPP] 1. 사용자가 먼저 WhatsApp으로 메시지를 보내야 합니다.`,
        );
        console.error(
          `[TWILIO_WHATSAPP] 2. 또는 프로덕션 WhatsApp Business API로 전환하세요.`,
        );
      }
    }
    ...
    if (error?.code === 20003) {
      ...
      console.error(
        "[TWILIO_WHATSAPP] Account SID 또는 Auth Token이 잘못되었거나 만료되었습니다.",
      );
      console.error("[TWILIO_WHATSAPP] 해결 방법:");
      console.error(
        "[TWILIO_WHATSAPP] 1. Twilio 콘솔(https://console.twilio.com/)에서 Account SID와 Auth Token을 확인하세요.",
      );
      console.error(
        "[TWILIO_WHATSAPP] 2. .env.local 파일의 TWILIO_ACCOUNT_SID와 TWILIO_AUTH_TOKEN을 업데이트하세요.",
      );
      console.error(
        "[TWILIO_WHATSAPP] 3. Auth Token이 만료되었을 수 있으니 새로 생성하세요.",
      );
      console.error("[TWILIO_WHATSAPP] 4. 개발 서버를 재시작하세요.");
    } else if (error?.code === 21660) {
      console.error(
        "[TWILIO_WHATSAPP] ⚠️  에러 21660: 발신번호와 계정이 일치하지 않습니다.",
      );
      console.error("[TWILIO_WHATSAPP] 해결 방법:");
      console.error(
        "[TWILIO_WHATSAPP] 1. Twilio 콘솔(https://console.twilio.com/)에서 현재 계정의 WhatsApp 전화번호를 확인하세요.",
      );
      console.error(
        "[TWILIO_WHATSAPP] 2. .env.local의 TWILIO_WHATSAPP_FROM을 계정에 등록된 번호로 변경하세요.",
      );
      console.error(
        "[TWILIO_WHATSAPP] 3. 또는 Twilio 콘솔에서 WhatsApp 전화번호를 구매/등록하세요.",
      );
    }
  ...
  try {
    ...
    for (const phoneNumber of accountInfo.phoneNumbers) {
      ...
      if (fromCountry === toCountry) {
        console.log(
          `[TWILIO_SMS] 같은 국가 매칭: ${phoneNumber} (${fromCountry})`,
        );
        return phoneNumber;
      }
    }
...
export function formatPhoneNumber(
  ...
  if (digits.length >= 11) {
    ...
    if (
      ["52", "51", "56", "57", "54", "58", "55", "53", "81", "86"].includes(
        twoDigitCode,
      )
    ) {
      return `+${digits}`;
