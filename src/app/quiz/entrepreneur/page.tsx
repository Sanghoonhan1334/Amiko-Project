...
export default function EntrepreneurTestPage() {
  ...
  // Slug del test Entrepreneur
  const ENTREPRENEUR_QUIZ_SLUG = 'entrepreneur'

  // Cargar datos del quiz
  useEffect(() => {
    const loadQuizData = async () => {
      try {
        const response = await fetch(`/api/quizzes/${ENTREPRENEUR_QUIZ_SLUG}`)
        if (!response.ok) {
  ...
  const handleStart = () => {
    ...
    router.push('/quiz/entrepreneur/start')
  }
  ...
  const handleShare = async () => {
    try {
      ...
      const shareUrl = `${baseUrl}/quiz/entrepreneur`
      const shareText = `${quizData?.description || '¿Qué tipo de emprendedor eres?'}\n\n${shareUrl}`
      
      if (navigator.share) {
        await navigator.share({
          title: quizData?.title || '¿Qué tipo de emprendedor eres?',
          text: shareText
        })
      } else {
          ...
          <div className="bg-white">
            ...
            <div className="mb-6">
              <div className="relative w-full h-64 rounded-lg overflow-hidden">
                <img 
                  src={quizData.thumbnail_url || "/quizzes/entrepreneur/cover.png"} 
                  alt={quizData.title}
