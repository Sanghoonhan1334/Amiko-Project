...
export async function GET(request: NextRequest) {
  try {
    ...
    const currentUserId = searchParams.get("userId") || null; // 현재 사용자 ID (자신의 pending 비디오 포함용)

    console.log("[DANCE_VIDEOS] 비디오 조회 시작:", {
      limit,
      status,
      includeGuide,
      currentUserId,
    ...
    if (includeGuide) {
      try {
        ...
        if (!adminError && adminUsers && adminUsers.length > 0) {
          ...
          console.log(
            "[DANCE_VIDEOS] 운영자 ID 조회 성공 (users 테이블):",
            adminUserIds.length,
            "명",
          );
        } else if (adminError) {
          console.warn(
            "[DANCE_VIDEOS] users 테이블 조회 실패, supabaseServer 시도:",
            adminError.message,
          );

          // 방법 2: supabaseServer 사용 (RLS 우회)
          if (supabaseServer) {
            const { data: adminUsersServer, error: adminErrorServer } =
            ...
            if (
              !adminErrorServer &&
              adminUsersServer &&
              adminUsersServer.length > 0
            ) {
              adminUserIds = adminUsersServer.map((au) => au.id);
              console.log(
                "[DANCE_VIDEOS] 운영자 ID 조회 성공 (supabaseServer):",
                adminUserIds.length,
                "명",
              );
            }
        ...
        console.warn(
          "[DANCE_VIDEOS] 운영자 정보 조회 실패 (무시하고 계속 진행):",
          adminErr,
        );
      }
    }
    ...
    // Helper para consultar videos
    const dbClient = supabaseServer || supabase;

    // 1. Obtener videos aprobados
    const { data: approvedVideos, error: approvedError } = await dbClient
      .from("dance_videos")
      .select("*")
      .eq("status", status)
    ...
    if (!approvedError) {
      ...
      console.log("[DANCE_VIDEOS] 비디오 조회 성공:", videos.length, "개");
    } else {
      error = approvedError;
      console.error("[DANCE_VIDEOS] 비디오 조회 실패:", approvedError.message);
    }

    // 2. Si hay userId, incluir también los videos pending del usuario actual
    if (currentUserId && !error) {
      const { data: pendingVideos, error: pendingError } = await dbClient
        .from("dance_videos")
        .select("*")
      ...
      if (!pendingError && pendingVideos && pendingVideos.length > 0) {
        console.log(
          "[DANCE_VIDEOS] 사용자 pending 비디오:",
          pendingVideos.length,
          "개",
        );
        // Agregar videos pending del usuario que no estén duplicados
        const existingIds = new Set((videos || []).map((v: any) => v.id));
        const newPending = pendingVideos
          .filter((v: any) => !existingIds.has(v.id))
          .map((v: any) => ({ ...v, is_own_pending: true }));
        videos = [...newPending, ...(videos || [])];
      }
    }
    ...
    if (error) {
      ...
      if (
        ...
        console.warn(
          "[DANCE_VIDEOS] dance_videos 테이블이 없습니다. 빈 배열 반환.",
        );
    ...
    const usersMap: {
      [key: string]: { display_name: string; avatar_url: string | null };
    } = {};
              ...
              if (displayName === "Anónimo") {
                try {
                  ...
                  if (!userError && userData) {
                    displayName =
                      userData.nickname || userData.full_name || "Anónimo";
    ...
    let processedVideos = videoList.map((video) => {
      const userInfo = usersMap[video.user_id] || {
        display_name: "Anónimo",
        avatar_url: null,
      };
      return {
        ...
        is_guide:
          ...
          adminUserIds.includes(video.user_id),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      data: { user },
      error: authError,
    } = await supabase.auth.getUser(decodedToken);

    if (authError || !user) {
      console.error("[DANCE_VIDEOS] 인증 실패:", authError?.message);
      return NextResponse.json(
        { error: "인증이 필요합니다." },
        { status: 401 },
      );
    ...
    // Usar supabaseServer (service role) para bypass RLS en operaciones de DB
    const dbClient = supabaseServer || supabase;

    // 현재 플레이리스트 가져오기 (playlist_id가 없으면)
    let finalPlaylistId = playlist_id;
    if (!finalPlaylistId) {
      const { data: currentPlaylist } = await dbClient
    ...
    const { data: video, error } = await dbClient
      .from("dance_videos")
      .insert({
        user_id: user.id,
    ...
    return NextResponse.json({ error: "비디오 업로드 실패" }, { status: 500 });
  }
}
