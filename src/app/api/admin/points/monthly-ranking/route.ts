...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[MONTHLY RANKING API] 시작')
    ...
    console.log('[MONTHLY RANKING] 조회할 user_ids:', userIds)
    ...
    console.log('[MONTHLY RANKING] users 조회 결과:', { usersData, usersError })
    ...
    console.error('월별 랭킹 조회 오류:', error)
    ...
    return NextResponse.json(
      { error: '월별 랭킹 조회 중 오류가 발생했습니다.', details: error?.message },
      { status: 500 }
    )
  }
}
