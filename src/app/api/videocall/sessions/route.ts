...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, topic, description, category, language, level, scheduled_at,
      duration_minutes, price_usd, max_participants, tags } = body

    if (!title || !topic || !scheduled_at) {
      return NextResponse.json({ error: 'title, topic, and scheduled_at are required' }, { status: 400 })
    }
    ...
    if (!hostProfile) {
      ...
      const { data: newHost, error: createError } = await supabase
        ...
        .insert({
          user_id: user.id,
          display_name: userProfile?.full_name || user.email?.split('@')[0] || 'Host',
      ...
      if (createError) {
        console.error('[VC_SESSIONS] Host profile creation error:', createError)
