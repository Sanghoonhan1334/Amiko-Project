  ...
  try {
    ...
    if (asHost) {
      ...
      query = supabase
        ...
        `)
        .in('session_id', 
          supabase.from('vc_sessions')
            .select('id')
            .in('host_id',
              supabase.from('vc_host_profiles').select('id').eq('user_id', user.id)
            )
        )
    } else {
      query = supabase
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { session_id, paypal_order_id, amount_paid } = body
    ...
    const totalAmount = amount_paid || session.price_usd
    ...
    const { data: booking, error } = await supabase
      ...
      .insert({
        ...
        payment_status: paypal_order_id ? 'paid' : 'pending',
        status: paypal_order_id ? 'confirmed' : 'reserved',
      })
      .select()
      .single()
