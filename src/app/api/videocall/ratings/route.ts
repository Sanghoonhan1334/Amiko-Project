...
export async function POST(request: NextRequest) {
  try {
    ...
    const { session_id, knowledge_rating, clarity_rating, friendliness_rating,
      usefulness_rating, comment } = body
    ...
    if (!booking) {
      return NextResponse.json({ error: 'Must participate to rate' }, { status: 403 })
    }

    // Calculate overall
    const ratings = [knowledge_rating, clarity_rating, friendliness_rating, usefulness_rating]
      .filter(r => r != null)
