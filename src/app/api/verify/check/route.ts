...
export async function POST(request: NextRequest) {
  try {
    ...
    // 채널 유효성 검증
    const validChannels = ["wa", "whatsapp", "sms", "email"];
    ...
    // DB type 변환: 'wa'/'whatsapp' → 'sms' (저장 시와 동일하게)
    const dbType = channel === "wa" || channel === "whatsapp" ? "sms" : channel;
    ...
    if (channel === "email") {
      ...
      console.log("[VERIFY_CHECK] 이메일 정규화:", {
        original: target,
      ...
      if (target.startsWith("+")) {
        ...
        if (!normalizedTarget.startsWith("+")) {
          console.warn("[VERIFY_CHECK] 전화번호 정규화 실패, 원본 사용:", {
            target,
            nationality,
            normalizedTarget,
          });
          ...
          console.log("[VERIFY_CHECK] 전화번호 정규화 성공:", {
            target,
            nationality,
            normalizedTarget,
          });
    ...
    if (channel === "email") {
      ...
      if (!userFindError && userData?.id) {
        ...
        const { error: authStatusError } = await supabase.rpc(
      ...
      if (nationality) {
        try {
          ...
          if (e164Phone && e164Phone.startsWith("+")) {
            ...
            console.log("[VERIFY_CHECK] 전화번호 E.164 정규화 성공:", {
              original: target,
      ...
      if (!userId) {
        ...
        const { data: userData, error: userFindError } = await supabase
          ...
          .select("id, phone")
          .or(
            `phone.eq.${target},phone.eq.${cleanPhone},phone.eq.${phoneWithPlus},phone.eq.${normalizedPhone}`,
          )
          .maybeSingle();

        if (!userFindError && userData?.id) {
          userId = userData.id;
          console.log("[VERIFY_CHECK] 전화번호로 사용자 찾기 성공:", {
            userId,
            phone: userData.phone,
            searched: [target, cleanPhone, phoneWithPlus, normalizedPhone],
          });
        } else {
          console.warn("[VERIFY_CHECK] 전화번호로 사용자 찾기 실패:", {
            target,
            cleanPhone,
            phoneWithPlus,
            normalizedPhone,
      ...
      if (userId) {
        ...
        if (existingUser) {
          ...
          console.log(
            "[VERIFY_CHECK] public.users에 사용자 없음, 생성 시도:",
            userId,
          );
        ...
        if (updateUserError) {
          console.error(
            "[VERIFY_CHECK] SMS 인증 상태 업데이트/생성 실패:",
            updateUserError,
          );
        } else {
          console.log("[VERIFY_CHECK] SMS 인증 상태 업데이트/생성 성공:", {
            userId,
            updateData,
          });
        }
        ...
        console.warn(
          "[VERIFY_CHECK] SMS 인증 완료했지만 사용자를 찾을 수 없음:",
          {
            target,
            cleanPhone,
            phoneWithPlus,
