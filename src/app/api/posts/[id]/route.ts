...
export async function GET(
  ...
  try {
    ...
    // skipView 파라미터 확인
    const skipView = request.nextUrl.searchParams.get("skipView") === "true";

    // 조회수 증가 (skipView가 아닌 경우에만)
    if (!skipView) {
    ...
    if (post.user_id) {
      ...
      console.log("[POST_GET] user_profiles 조회 결과:", {
        profileData,
        profileError,
      });
      ...
      if (!author) {
        console.log(
          "[POST_GET] user_profiles에서 author를 찾지 못함, users 테이블 조회",
        );
        const { data: userData } = await supabaseServer
          .from("users")
          .select(
            "id, email, full_name, nickname, spanish_name, korean_name, profile_image, avatar_url, is_admin",
          )
          .eq("id", post.user_id)
          .single();
        ...
        if (userData) {
          ...
          let finalName =
            userData.korean_name || userData.spanish_name || userData.full_name;
    ...
    const responseData = {
      ...
      view_count: skipView ? post.view_count || 0 : (post.view_count || 0) + 1, // 증가된 조회수 (skipView 시 원래 값 유지)
