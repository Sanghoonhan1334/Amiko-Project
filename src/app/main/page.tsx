...
const CommunityTab = dynamic(
  () => import("@/components/main/app/community/CommunityTab"),
  {
    loading: () => (
      <div className="space-y-4 p-4">
...
function AppPageContent() {
  ...
  const checkAdminStatus = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/admin/check?${params.toString()}`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    ...
    if (
      tabParam &&
      ["home", "meet", "community", "me", "event"].includes(tabParam)
    ) {
  ...
  return (
    <div className="min-h-screen body-gradient dark:bg-gray-900 pb-20 md:pb-0">
      {/* 메인 콘텐츠 섹션 */}
      <div className="w-inside mx-auto px-2 relative z-0">
                        ...
                        {(() => {
                          const title =
                            communityView === "home"
                              ...
                              : communityView === "news" ||
                                communityView === "news-detail"
                              ? t("community.koreanNews")
                              : communityView === "qa"
                              ? t("community.qa")
                              : communityView === "tests"
                              ? t("tests.title")
                              : t("main.community");
                          console.log("헤더 제목 디버그:", {
                            communityView,
                            title,
                            koreanNews: t("community.koreanNews"),
                          });
                          return title;
                        })()}
                      </h2>
                      ...
                      <button
                        onClick={() => {
                          ...
                          window.dispatchEvent(
                            new CustomEvent("communityViewChanged", {
                              detail: "home",
                            })
                          );
                        }}
                  ...
                  <div className="mb-6">
                    <p className="text-gray-600 dark:text-gray-400">
                      {communityView === "home"
                        ...
                        : communityView === "news"
                        ? t("community.koreanNewsDescription")
                        : communityView === "qa"
                        ? t("community.qaDescription")
                        : communityView === "tests"
                        ? t("tests.description")
                        : t("main.communityDescription")}
                    </p>
